...
type wishlistServiceImpl struct {
	store    repository.WishlistRepository
	maxItems int
}

// constructor
func NewWishlistService(store repository.WishlistRepository) WishlistService {
...
func (s *wishlistServiceImpl) GetWishlist(ctx context.Context, accountID uuid.UUID, page int, size int, keyword string) ([]uuid.UUID, int, error) {
	w, total, err := s.store.FindWishlistProductIDsByAccountID(ctx, accountID, keyword, size, (page-1)*size)
	if err != nil {
...
func (s *wishlistServiceImpl) AddToWishlist(ctx context.Context, accountID uuid.UUID, productID uuid.UUID) error {
	...
	existingWishlist, _, err := s.store.FindWishlistByAccountIDAll(ctx, accountID)
	if err != nil {
	...
	// 4️⃣ Save to DB
	return db_error.HandlePgError(s.store.SaveWishlistItem(ctx, accountID, productID))
}
...
func (s *wishlistServiceImpl) RemoveFromWishlist(ctx context.Context, accountID uuid.UUID, productID uuid.UUID) error {
	return db_error.HandlePgError(s.store.DeleteWishlistItem(ctx, accountID, productID))
}
